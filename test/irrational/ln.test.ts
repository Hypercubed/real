...
const ln = (v: any) => Irrational.from(v).ln();
...
test('error propagation', () => {
  expect(ln(
    ...
    Irrational.from(1.39).withError(0.03));

  expect(ln(
    Irrational.from(8).withError(0.005)
    )).toEqual(
    Irrational.from(2.0794).withError(0.0007));

  expect(ln(
    Irrational.from(0.1).withError(0.009)
    )).toEqual(
      Irrational.from(-2.30).withError(0.09));
});
...
test('basics', () => {
  ...
  expect(	ln	(	'9999.'	      ).toString()).toBe(	'9.2102e+0'	);
  expect(	ln	(	'9999.000000'	).toString()).toBe(	'9.2102403670e+0'	);
});

test('fractions', () => {
                                                    // -7.264430222920869430966613075294277100767321224933230
  expect(	ln	(	'0.000700000'	    ).toString()).toBe(	'-7.264430e+0'	);
...
test('ln near E', () => {  // TODO: slow
  ...
  // expect(	ln	(	'2.718281828459045'	).toString()).toBe(	'9.999999999999999e-1'	);  // slow
  ...
  expect(ln('2718.').toString()).toBe('7.9077e+0');
...
test('ln powers of ten', () => {
  ...
  expect(	ln	(	'1.00000000E6'	).toString()).toBe(	'1.381551056e+1'	);
                                                  // 1.381551055796427410410794872810618524560660893177263785619

                                                     // 2.0723265836946411156161923092159277868409913397658956
  expect(	ln	(	'1.00000000e+9'	  ).toString()).toBe(	 '2.072326584e+1'	);
  expect(	ln	(	'1.000000000e-9'	).toString()).toBe(	'-2.0723265837e+1'	);
});
